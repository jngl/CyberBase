...
template <class T> struct Matrix4
{
    ...
    void projectPerspective(float pFOV, float pAspectRatio, float pNear,
        ...
        // Z coordinate makes z -1 when we're on the near plane and +1 on the
        // far plane
        m[2][2] = (pNear + pFar) / (pNear - pFar);
        m[3][2] = 2 * pNear * pFar / (pNear - pFar);

        // W = -1 so that we have [x y z -z], a homogenous vector that becomes
        // [-x/z -y/z -1] after division by w.
        m[2][3] = -1;
    ...
    void addScale(T x, T y, T z) { addScale(Vector3<T>(x, y, z)); }
